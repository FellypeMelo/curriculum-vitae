...
export function TiltCard({ children, className = '', featured = false }: TiltCardProps) {
  ...
  const onPointerMove = (e: PointerEvent<HTMLDivElement>) => {
    ...
    cardRef.current.style.transition = 'transform 0.08s ease-out, border-color 0.25s ease';
    cardRef.current.style.setProperty('--mouse-x', `${x}px`);
    cardRef.current.style.setProperty('--mouse-y', `${y}px`);
    cardRef.current.style.transform = `perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg) translateZ(${featured ? '6px' : '0px'})`;
  };

  const onPointerLeave = () => {
    if (!cardRef.current) return;
    cardRef.current.style.transition = 'transform 0.5s cubic-bezier(0.16, 1, 0.3, 1), border-color 0.25s ease';
