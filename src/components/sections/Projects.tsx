  ...
  return (
    <section id="projetos" className="border-b border-border py-24 md:py-32">
      <div className="mx-auto max-w-[1240px] px-5 md:px-8">
        ...
        <div className="mt-14 grid grid-cols-1 gap-5 lg:grid-cols-[1.12fr_0.88fr]">
          {PROJECTS.map((p, i) => {
            const featured = i === 0;
            const Icon = p.icon;
            return (
              <Reveal key={p.name} delay={i * 0.08}>
                <article
                  className={`group relative flex h-full flex-col overflow-hidden border border-border p-7 transition-colors hover:border-accent/50 md:p-9 ${
                    featured ? 'bg-surface/40' : 'bg-surface/20'
                  }`}
                >
                  {featured && (
                  ...
                  <ul className="relative mt-auto flex flex-wrap gap-2 pt-7">
                    {p.stack.map((s) => (
                      <li
                        key={s}
                        className="border border-border bg-bg/60 px-2.5 py-1 font-mono text-[0.7rem] tracking-tight text-dim"
                      >
                        {s}
                      </li>
                    ))}
